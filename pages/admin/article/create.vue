      ...
      <div class="col-9">
        <div class="form__wrapper">
          <form class="form bg-light">
            <div class="form__title">Создать статью</div>
            <no-ssr>
              <Field
                v-if="$v"
                :error="$v.title.$error"
  ...
  methods: {
    ...
    sendForm() {
      ...
      if(this.getToken) {
        ...
        .then(({ data }) => {
          if (data) {
            this.$router.push(`/article/${data.articles_slug}`)
          } else {
            this.errorForm = true
          }
          this.loading = false
    ...
    selectCategory(categories) {
      this.articleCategories = categories.map(item => item.id)
    },
